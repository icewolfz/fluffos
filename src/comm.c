   ...
   num_bytes = write(ip->fd,ip->message_buf + ip->message_consumer,length);
		...
		if (ip->compressed_stream) {
			...
			if(ip->connection_type == PORT_WEBSOCKET && (ip->iflags & HANDSHAKE_COMPLETE)){
				...
				int sendres;
				//Icewolfz@shadowmud recoded to add larger packet size up to 65535 chunks
				if(length <= 125)
				{
					flags = htons(sendsize | 0x8200); //82 is final packet (of this message) type binary
					sendres = send(ip->fd, &flags, 2, 0);
				}
				else
				{
					//if greater then 65535 set it to 65535 
					if(length > 65535)
						sendsize = 65535;
					flags = htons(0x827E); //82 is final packet (of this message) type binary with 126 marking it as size 126 to 65535
					sendres = send(ip->fd, &flags, 2, 0);
					flags = htons((unsigned short)sendsize);
					sendres = send(ip->fd, &flags, 2, 0);						
				}
				if(sendres <= 0)
					return 1; //wait
	...
	case PORT_WEBSOCKET:
		if(ip->iflags & HANDSHAKE_COMPLETE){
			...
			if(!ip->ws_size){
				...
				ip->ws_maskoffs = 0;
				//Icewolfz@shadowmud - attempt to ignore pong frame to fix IE/Edge browsers
				if(op == 9 || op == 10)
				{
					if(size > ip->ws_text_end-ip->ws_text_start)
						ip->ws_size = ip->ws_text_end-ip->ws_text_start;					
					ip->ws_text_start += ip->ws_size;
					ip->ws_size = 0;
					break;
				}				
			}
...
static int flush_compressed_output (interactive_t *ip) {
	...
	if (len > 0) {
		...
		for (iStart = 0; iStart < len; iStart += nWrite)
		{
			...
			//Icewolfz@shadowmud added MCCP support for websocket 
			if(ip->connection_type == PORT_WEBSOCKET && (ip->iflags & HANDSHAKE_COMPLETE))
			{
				int sendres;
				int sendsize = nBlock;	
				...
				{
					//no need to check if sendsize is greater then 65535 as nBlock is never over 4096
					flags = htons(0x827E); //82 is final packet (of this message) type binary with 126 marking it as size 126 to 65535
					sendres = send(ip->fd, &flags, 2, 0);
					flags = htons((unsigned short)sendsize);
					sendres = send(ip->fd, &flags, 2, 0);
